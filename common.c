...
int TERM_COLOR = false;

char *str_dup(const char *s) //allocates new string
{
    if (!s)
        return NULL;
...
unsigned strsubct(const char *str, const char *sub) //no side effects
{
    unsigned count = 0;
    if (str && *str && sub && *sub) {
...
char *strrepl(const char *str, const char *find, const char *repl) //allocates new string
{
...
char *next_tok(char **line) //modifies line double ptr, copies token
{
    if (!line || !*line || !**line)
        return NULL;
...
char *rmqt(const char *str) //allocates new string
{
...
char *addqt(const char *str) //allocates new string
{
    if (!str)
        return NULL;
...
void *add_element(void *base, size_t *nmemb, size_t size, unsigned i, void *new_elem)
{
    ...
    memmove((char *)base + (i + 1) * size,
            (char *)base + i * size,
            (*nmemb - (i + 1)) * size);
    memcpy((char *)base + i * size, new_elem, size);
    return base;
}
